    ...
    with open('dataset2.csv', 'w', encoding='UTF8', newline='') as f:
        ...
        for key in a.get_keys():
            ...
            row = [art.title, art.url, art.source_domain, art.description, art.authors,
                   art.date_published, art.main_text]
            writer.writerow(row)
...
def clean_dataset(arts: Articles) -> None:
    ...
    for key in arts.get_keys():
        ...
        #     article.authors[i] = fix_unicode(article.authors[i])
...
if __name__ == '__main__':
    ...
    scraped_articles = NewsPlease.from_file('links.txt')
    print('scraping complete')
    articles = setup_articles(scraped_articles)
